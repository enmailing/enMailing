...
import com.enmailing.k9.Account.QuoteStyle;
import com.enmailing.k9.EmailAddressAdapter;
import com.enmailing.k9.EmailAddressValidator;
import com.enmailing.k9.FontSizes;
import com.enmailing.k9.Identity;
import com.enmailing.k9.K9;
import com.enmailing.k9.Preferences;
import com.enmailing.k9.R;
import com.enmailing.k9.Account.MessageFormat;
import com.enmailing.k9.Account.QuoteStyle;
import com.enmailing.k9.activity.loader.AttachmentContentLoader;
import com.enmailing.k9.activity.loader.AttachmentInfoLoader;
import com.enmailing.k9.activity.misc.Attachment;
import com.enmailing.k9.controller.MessagingController;
import com.enmailing.k9.controller.MessagingListener;
import com.enmailing.k9.crypto.CryptoProvider;
import com.enmailing.k9.crypto.OpenPgpApiHelper;
import com.enmailing.k9.crypto.PgpData;
import com.enmailing.k9.fragment.ProgressDialogFragment;
import com.enmailing.k9.helper.ContactItem;
import com.enmailing.k9.helper.Contacts;
import com.enmailing.k9.helper.HtmlConverter;
import com.enmailing.k9.helper.IdentityHelper;
import com.enmailing.k9.helper.StringUtils;
...
import com.enmailing.k9.mail.Multipart;
import com.enmailing.k9.mail.Part;
import com.enmailing.k9.mail.Message.RecipientType;
import com.enmailing.k9.mail.internet.MimeBodyPart;
import com.enmailing.k9.mail.internet.MimeHeader;
import com.enmailing.k9.mail.internet.MimeMessage;
import com.enmailing.k9.mail.internet.TextBodyBuilder;
...
import com.enmailing.k9.mail.store.LocalStore.LocalAttachmentBody;
import com.enmailing.k9.mail.store.LocalStore.TempFileBody;
import com.enmailing.k9.mail.store.LocalStore.TempFileMessageBody;
...
public class MessageCompose extends K9Activity implements OnClickListener,
    ...
    private static final String ACTION_COMPOSE = "com.enmailing.k9.intent.action.COMPOSE";
    private static final String ACTION_REPLY = "com.enmailing.k9.intent.action.REPLY";
    private static final String ACTION_REPLY_ALL = "com.enmailing.k9.intent.action.REPLY_ALL";
    private static final String ACTION_FORWARD = "com.enmailing.k9.intent.action.FORWARD";
    private static final String ACTION_EDIT_DRAFT = "com.enmailing.k9.intent.action.EDIT_DRAFT";
    ...
    private static final String STATE_KEY_ATTACHMENTS =
        "com.enmailing.k9.activity.MessageCompose.attachments";
    private static final String STATE_KEY_CC_SHOWN =
        "com.enmailing.k9.activity.MessageCompose.ccShown";
    private static final String STATE_KEY_BCC_SHOWN =
        "com.enmailing.k9.activity.MessageCompose.bccShown";
    private static final String STATE_KEY_QUOTED_TEXT_MODE =
        "com.enmailing.k9.activity.MessageCompose.QuotedTextShown";
    private static final String STATE_KEY_SOURCE_MESSAGE_PROCED =
        "com.enmailing.k9.activity.MessageCompose.stateKeySourceMessageProced";
    private static final String STATE_KEY_DRAFT_ID = "com.enmailing.k9.activity.MessageCompose.draftId";
    private static final String STATE_KEY_HTML_QUOTE = "com.enmailing.k9.activity.MessageCompose.HTMLQuote";
    private static final String STATE_IDENTITY_CHANGED =
        "com.enmailing.k9.activity.MessageCompose.identityChanged";
    private static final String STATE_IDENTITY =
        "com.enmailing.k9.activity.MessageCompose.identity";
    private static final String STATE_PGP_DATA = "pgpData";
    private static final String STATE_IN_REPLY_TO = "com.enmailing.k9.activity.MessageCompose.inReplyTo";
    private static final String STATE_REFERENCES = "com.enmailing.k9.activity.MessageCompose.references";
    private static final String STATE_KEY_READ_RECEIPT = "com.enmailing.k9.activity.MessageCompose.messageReadReceipt";
    private static final String STATE_KEY_DRAFT_NEEDS_SAVING = "com.enmailing.k9.activity.MessageCompose.mDraftNeedsSaving";
    private static final String STATE_KEY_FORCE_PLAIN_TEXT =
            "com.enmailing.k9.activity.MessageCompose.forcePlainText";
    ...
    private EnMailing mEnMailing = null;
    private LinearLayout mEnMailingLayout;
    private Button mEnMailingEncryptButton;
    private Button mEnMailingDecryptButton;
    private Spinner mEnMailingGroupSpinner;
    private Button mEnMailingGroupRefreshButton;
    private CheckBox mEnMailingEncryptCheck;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mEnMailing = new EnMailing(this);
        mEnMailingLayout = (LinearLayout)findViewById(R.id.layout_enmailing);
        mEnMailingEncryptButton = (Button)findViewById(R.id.enmailing_encrypt);
        mEnMailingEncryptButton.setOnClickListener(new OnClickListener() {
        	@Override
        	public void onClick(View view) {
				mEnMailing.encryptMessage(MessageCompose.this, EnMailing.getGroup());
				mEnMailingEncryptCheck.setChecked(false);
				mEnMailingDecryptButton.setVisibility(View.VISIBLE);
				mPreventDraftSaving = true;
        	}
        });
        mEnMailingEncryptButton.setVisibility(View.GONE);
        
        mEnMailingDecryptButton = (Button)findViewById(R.id.enmailing_decrypt);
        mEnMailingDecryptButton.setOnClickListener(new OnClickListener() {
        	@Override
        	public void onClick(View view) {
        		final String text = mMessageContentView.getText().toString();
        		if (EnMailing.hasEncryptions(text)) {
        			mEnMailing.decryptMessage(MessageCompose.this);
        			mEnMailingDecryptButton.setVisibility(View.GONE);
        		}
        	}
        });
        mEnMailingDecryptButton.setVisibility(View.GONE);
        
        mEnMailingGroupSpinner = (Spinner)findViewById(R.id.enmailing_groups);
        List<String> list=new ArrayList<String>();
        //EnMailing enMailing = new EnMailing(this);
        //String strGroups = mEnMailing.getSavedGroups();
        //String[] arrGroups = strGroups.split("\\|");
        ArrayList<String> arrGroups = EnMailing.getSavedGroups();
        String group = EnMailing.getGroup();
        int grpPos = 0;
        for (int i=0; i < arrGroups.size(); i++) {
        	String grp = arrGroups.get(i);//arrGroups[i];
        ...
        mEnMailingGroupSpinner.setAdapter(adapter);
        mEnMailingGroupSpinner.setSelection(grpPos);
        mEnMailingGroupSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
        	@Override
        	public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {
                //EnMailing enMailing = new EnMailing(MessageCompose.this);
                EnMailing.setGroup(mEnMailingGroupSpinner.getItemAtPosition(position).toString());
            }
        ...
        mEnMailingGroupRefreshButton = (Button)findViewById(R.id.enmailing_refresh_groups);
        mEnMailingGroupRefreshButton.setOnClickListener(new OnClickListener() {
        	@Override
        	public void onClick(View v) {
        		//EnMailing enMailing = new EnMailing(MessageCompose.this);
        		mEnMailing.refreshGroups(mEnMailingGroupSpinner);
        	}
        });
        
        mEnMailingEncryptCheck = (CheckBox)findViewById(R.id.enmailing_auto_encrypt);
        mEnMailingEncryptCheck.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
        	@Override
			public void onCheckedChanged(CompoundButton buttonView,
				...
				if (isChecked) {
        			...
        			if (!EnMailing.hasEncryptions(mQuotedHtmlContent.toString())) {
        ...
        mMessageContentView.setCustomSelectionActionModeCallback(new ActionMode.Callback() {
			@Override
			...
			public boolean onCreateActionMode(ActionMode mode, android.view.Menu menu) {
		        ...
		        inflater.inflate(R.menu.message_compose_action, menu);
				return true;
			}
			...
			public boolean onActionItemClicked(ActionMode mode,
		        ...
		        //SpannableStringBuilder ssb = new SpannableStringBuilder(mMessageContentView.getText());

		        switch(item.getItemId()) {

		        case R.id.message_compose_action_encrypt:
		        	mEnMailing.encryptSelection(mMessageContentView, mEnMailing.getGroup());
		        	mEnMailingEncryptCheck.setChecked(false);
		        	mEnMailingDecryptButton.setVisibility(View.VISIBLE);
		        	mPreventDraftSaving = true;
		            return true;
    ...
    protected void onRestoreInstanceState(Bundle savedInstanceState) {
        ...
        if (mQuotedHtmlContent != null && mQuotedHtmlContent.getQuotedContent() != null) {
            ...
            if (mQuotedHTML.isEnMailingMessage()) {
            	prepareEnMailing();
            }
        }
     ...
     */
    public TextBody buildText(boolean isDraft) {
    ...
    private TextBody buildText(boolean isDraft, SimpleMessageFormat messageFormat) {
        ...
        if (includeQuotedText) {
        	if (mEnMailingEncryptCheck.isChecked() && !isDraft) {
        		if (EnMailing.hasEncryptions(mQuotedHtmlContent.toString())) {
            		StringBuilder builder = new StringBuilder(mEnMailing.decryptString(mQuotedHtmlContent.toString()));
            		mQuotedHtmlContent.setQuotedContent(builder);
            		int offset = mQuotedHtmlContent.toString().length() - mQuotedHtmlContent.getFooterInsertionPoint();
            		if (offset > 0) {
            			mQuotedHtmlContent.setFooterInsertionPoint(builder.length()-offset);
            		} else {
            			mQuotedHtmlContent.setFooterInsertionPoint(builder.length());
            		}
            		//mQuotedHtmlContent.setHeaderInsertionPoint(0);
            	}
        	}
        ...
        if (mEnMailingEncryptCheck.isChecked() && !isDraft) {
        	...
        	if (body.getText().contains("<html>")) {
        		text = text.replaceFirst("<html><head></head><body>", "").replaceFirst("</body></html>", "");
        		text = mEnMailing.encryptStringSync(text, EnMailing.getGroup());
        		text = "<html><head></head><body>"+text+"</body></html>";
        	} else {
        		text = mEnMailing.encryptStringSync(text, EnMailing.getGroup());        		
        	}
        	if (mEnMailing.lastEncryptSuccess()) {
        		body = new TextBody(text); 
				body.setComposedMessageLength(text.length());
                body.setComposedMessageOffset(0);
        	} else {
        		EnMailing.textBodyFailed(body);
        	}
        }
        return body;
    }
    ...
    private MimeMessage createMessage(boolean isDraft) throws MessagingException {
        ...
        if (mEnMailingEncryptCheck.isChecked() && !isDraft) {
        	if (EnMailing.textBodyFailed(bodyPlain)) {
        		EnMailing.textBodyToMessageFail(bodyPlain, message);
        	}
        }
    ...
    private void performSend() {
        ...
        /*if (mEnMailingEncryptCheck.isChecked()) {
        	//EnMailing enMailing = new EnMailing(this);
        	//String text = buildText(false).getText();
        	mEnMailing.encryptMessage(this, mEnMailing.getGroup(), true);
        } else {
        	sendMessage();
        }*/
        sendMessage();
    ...
    private void toggleEnMailingOptions() {
    	if (mEnMailingLayout.getVisibility() == View.GONE) {
    		mEnMailingLayout.setVisibility(View.VISIBLE);
    	} else {
    		mEnMailingLayout.setVisibility(View.GONE);
    	}
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.read_receipt:
                onReadReceipt();
                break;
            case R.id.enmailing_compose:
            	//Log.i(EnMailing.LOG_TAG, "Toggling enmailing options");
            	toggleEnMailingOptions();
            	break;
            default:
        ...
        if (textPart != null) {
            ...
            if (bodyLength > 0) {
                try {
                    ...
                    String quoteText = quotedText.toString();
                    if (!quoteText.isEmpty() && EnMailing.hasEncryptions(quoteText)) {
                    	mQuotedText.setCharacters(EnMailing.getLoadingMessage(quoteText));
            	    	quoteText = mEnMailing.decryptString(quoteText);
            	    	prepareEnMailing();
                	}
                    mQuotedText.setCharacters(quoteText);
    ...
    private void populateUIWithQuotedMessage(boolean showQuotedText) throws MessagingException {
        ...
        if (mQuotedTextFormat == SimpleMessageFormat.HTML) {
            ...
            String quoteText = mQuotedHtmlContent.getQuotedContent();
            if (!quoteText.isEmpty() && EnMailing.hasEncryptions(quoteText)) {
            	mQuotedHTML.setText(EnMailing.getLoadingMessage(quoteText));
    	    	mEnMailing.decryptMessage(mQuotedHTML, quoteText);
    	    	mQuotedHTML.setEnMailingMessage(true);
    	    	prepareEnMailing();
        	} else {
        		mQuotedHTML.setText(quoteText);
        	}
            ...
            String quoteText = quoteOriginalTextMessage(mSourceMessage, content, mQuoteStyle);
            if (!quoteText.isEmpty() && EnMailing.hasEncryptions(quoteText)) {
            	mQuotedText.setCharacters(EnMailing.getLoadingMessage(quoteText));
    	    	quoteText = mEnMailing.decryptString(quoteText);
    	    	prepareEnMailing();
        ...
        mEnMailing.decryptEditText(mQuotedText);
    }
    ...
    private class SendMessageTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                ...
                if (EnMailing.mimeMessageFailed(message)) {
                	Toast.makeText(MessageCompose.this, "Could not encrypt message. Please check your enMailing settings.", Toast.LENGTH_LONG).show();
                	EnMailing.removeMimeMessageFail(message);
                	return null;
                }
                Log.e(EnMailing.LOG_TAG, "Created message: "+message.getBody().toString());
            } catch (MessagingException me) {
    ...
    public SimpleMessageFormat getMessageFormat() {
    ...
    private void prepareEnMailing() {
    	mEnMailingLayout.setVisibility(View.VISIBLE);
    	mEnMailingEncryptCheck.setChecked(true);
    	mPreventDraftSaving = true;
    }
    
    public EditText getMessageContent() {
